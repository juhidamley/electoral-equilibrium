...
def aggregate(maes: list[float], bloc_errors: dict[str, list[float]]) -> dict:
    overall = sum(maes) / len(maes) if maes else float("nan")
    per_bloc = {
        b: sum(errs) / len(errs)
...
def write_markdown(results: dict, path: Path) -> None:
    ...
    for method, data in results.items():
        lines.append(
            f"| {method} | {_fmt(data['overall_mae'])} | {data['n_records']} |"
        )
    ...
    lines += [f"*Generated by scripts/run_baseline_comparison.py*", ""]
...
def main() -> None:
    ...
    print("─" * 36)
    for method, data in results.items():
        print(f"{method:<14}  {_fmt(data['overall_mae']):>11}  {data['n_records']:>5}")
