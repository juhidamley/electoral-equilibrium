...
class ShockEstimator:
    ...
    def estimate(self, event: dict[str, Any], intensity: float = 1.0) -> Any:
        ...
        # Rescaled Step 2.1: old clip 0.15 was exactly the strong_pos/strong_neg
        # bin's outer edge (not its 0.12 midpoint); 0.0375 = 0.15 x0.25 preserves
        # that same relationship at the new scale, so the amount of headroom
        # `intensity` has above a bin's own midpoint before clipping engages is
        # unchanged. intensity is user-controllable via the API in [0.1, 3.0]
        # (shock_endpoint.py), so this clip is a real operative ceiling, not
        # vestigial -- it only fails to bind at intensity<=1.25.
