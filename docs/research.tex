...
are \emph{documented electorate shares}, not equal $1/n$ weights. We use
...
(1992--2024, yielding eight first-differences), not the full 1948--2024 panel:
...
implementation asserts disciplined quasi-convexity at runtime as a correctness guard.
...
infeasible---a feature, not a failure.
...
The backend is a FastAPI application served on Modal using a single A10G GPU (24\,GB).
...
DuckDB audit log with per-stage latencies. For cost control the service scales to zero
...
pointed at the Modal endpoint. A user selects a party, types a shock, and sets an
...
with a preview of the exact string sent to the model. Three widgets render the output:
(i) a plain-language \emph{narrative} that groups blocs into gaining, losing, and---
distinct from earlier versions---little-measurable-effect (neutral) buckets;
...
population share to prevent a common misreading; and (iii) a \emph{win-probability
gauge} with a percentile strip showing the 90\% credible interval. A share button
encodes the query into URL parameters. An authenticated analyst dashboard exposes six
...
convergence, and a searchable audit table.
