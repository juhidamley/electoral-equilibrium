...
diagnostics); the retrained adapter resolves a documented directional error in
which the pre-augmentation model predicted uniformly positive shifts for all
minority blocs, and is served behind a seeded, constrained-decoding inference path.
...
We further report a structural correctness result: weighting within-stratum
loyalties by documented electorate marginals rather than equal $1/n$ weights
removes a Democratic scoring bias of $+5.2$ points at zero shock, reducing the
baseline party asymmetry by 98\% ($+0.052 \rightarrow +0.001$) and satisfying a
bipartisan-neutrality requirement. Matched-pair symmetry testing on the corrected
baseline additionally isolates a residual $+0.028$ Democratic lean in the model's
own predictions; a three-way elimination analysis (aggregation, covariance,
training-corpus imbalance) localizes it to the base model's pretrained political
priors, which survive light LoRA adaptation, and we disclose it as a limitation
rather than tuning it away. The race-bloc covariance $\SigmaDelta$ is
estimated by Ledoit-Wolf shrinkage over a restricted nine-cycle window
(1992--2024) that suppresses spurious variance from sparse pre-1990 minority-bloc
cells (Asian standard deviation $0.243 \rightarrow 0.122$). Electoral College
asymmetry analysis places the Democrat win threshold at 50.66\% of the two-party
popular vote versus 49.34\% for Republicans; this threshold is an empirical
estimate subject to ongoing validation (Section~\ref{sec:veq_results}).
Finally, the full three-stage pipeline is deployed as a live web application:
a GPU-backed streaming inference service (Modal A10G) feeds a Next.js interface
that renders post-shock coalition weights, a win-probability gauge with credible
interval, and an analyst dashboard, returning results progressively as each
...
\begin{enumerate}
  ...
  \item A quasi-convexity proof for the Sharpe-ratio coalition objective and a
        ...
        The optimizer additionally carries per-bloc box constraints
        ($0.5 \times$--$1.5 \times$ the NEP electorate share), enforced as linear
        constraints inside the second-order cone program, which prevent the
        physically implausible single-bloc concentrations produced by the
        unconstrained formulation.

  \item A structural correctness result satisfying a bipartisan-neutrality
        requirement: weighting within-stratum loyalties by documented electorate
        marginals rather than equal $1/n$ weights removes a $+5.2$-point Democratic
        scoring bias measurable at zero shock, reducing the baseline party
        asymmetry by 98\% ($+0.052 \rightarrow +0.001$). We further show that a
        residual win-probability gap is not a bias but a modeled property---the
        Republican optimal coalition is more concentrated and therefore more
        electorally fragile---and validate directional symmetry via matched
        Democrat/Republican event pairs and known-valence events. Matched-pair
        testing on the corrected baseline additionally isolates a residual
        $+0.028$ Democratic lean in the model's own predictions; an elimination
        analysis across three candidate causes (aggregation, covariance,
        training-corpus imbalance) localizes it to the base model's pretrained
        political priors, which we disclose as a limitation rather than tune
        away (Section~\ref{sec:model_lean}).
  ...
  \item Fine-tuning results for the Stage~1 Mistral 7B QLoRA adapter: rank-16
        ...
        a smaller parameter budget. We document and \emph{resolve} a directional
        error on the Newsom 2028 nomination scenario: the pre-augmentation model
        predicted \texttt{slight\_pos} for all minority blocs, whereas the adapter
        retrained on the 1,136-record augmented dataset---served on production
        hardware behind a seeded, constrained-decoding path---differentiates
        correctly by bloc (White and Latino \texttt{slight\_neg}, Evangelical
        \texttt{mod\_neg}, Secular \texttt{slight\_pos}).

  \item A deployed, end-to-end system: the full three-stage pipeline runs as a
        live web application, with a GPU-backed streaming inference service
        (Modal A10G, server-sent events) feeding a Next.js interface that renders
        post-shock coalition weights, a win-probability gauge with credible
        interval, and an authenticated analyst dashboard.
\end{enumerate}
...
empirical findings, including the bipartisan-neutrality and symmetry-validation
results. Section~\ref{sec:system} describes the deployed web application.
Section~\ref{sec:discussion} discusses limitations, the ecological fallacy, and
calibration against prediction markets. Section~\ref{sec:conclusion} concludes.
...
\texttt{EquilibriumData}, \texttt{SimulationData})\footnote{Defined as frozen
dataclasses in \texttt{electoral/artifacts.py}.} written to disk in a
...
\text{stage\_name})$, ensuring byte-identical reproducibility across
runs.\footnote{\texttt{derive\_seed} and \texttt{make\_rng} are implemented in
\texttt{electoral/core/rng.py}.}
...
covering approximately 100\% of voters:\footnote{Canonical bloc identifiers are
defined in \texttt{electoral/core/types.py}; panel ingestion and validation are
implemented in \texttt{electoral/data/panel.py} and
\texttt{electoral/data/cleaning.py}.}
...
The three-stratum effective loyalty scalar is:\footnote{Computed by
\texttt{compute\_mu\_eff} in \texttt{electoral/optimization/dqcp.py}.}
...
and $\mathbf{g}$ are fixed and are not decision variables.

\textbf{Within-stratum marginals.} The fixed weights $\mathbf{v}$ and $\mathbf{g}$
are \emph{documented electorate shares}, not equal $1/n$
weights.\footnote{Stored as \texttt{\_APPROX\_RELIGION\_SHARE} and
\texttt{\_APPROX\_GENDER\_SHARE} in \texttt{electoral/models/ml\_baseline.py},
consumed by \texttt{compute\_fixed\_loyalty} in
\texttt{electoral/optimization/dqcp.py}.} We use
$\mathbf{v} = ($evangelical $0.24$, catholic $0.21$, protestant $0.13$, secular
$0.26$, jewish $0.02$, muslim $0.01$, other religion $0.13)$ from Pew Research and
ANES, and $\mathbf{g} = ($women $0.52$, men $0.47$, other gender $0.01)$ from the
Current Population Survey and ANES; each sums to one. This choice is consequential:
the earlier equal-weight placeholder ($v_r = 1/7$, $g_g = 1/3$) over-weighted small
but strongly Democratic blocs (Jewish, Muslim, other gender) and injected a constant
Democratic tailwind into $\mueff$ on \emph{every} scenario---a structural scoring bias
we diagnose and correct in Section~\ref{sec:symmetry_results}.
...
(Section~\ref{sec:raking}).\footnote{Implemented in
\texttt{electoral/kernels/raking.py}; the result is written to
\texttt{configs/layer\_weights.json}.} We maintain two calibrations in parallel: an
...
derive $\Veq$ by logistic regression:\footnote{Implemented as
\texttt{derive\_ec\_veq} in \texttt{electoral/models/ml\_baseline.py}; the
fitted values are cached in \texttt{configs/party\_config.json}.}
...
conflicts are resolved by inverse-standard-error weighting:\footnote{Implemented
in \texttt{electoral/kernels/data.py} (\texttt{\_agg\_stratum}), using Kish
effective sample size for ANES/GSS/CES weighting.}
\begin{equation}
...
Perot support from the 1992 National Exit Poll:\footnote{Implemented as
\texttt{correct\_three\_party\_1992} in
\texttt{electoral/models/ml\_baseline.py}.}
...
external sources with explicit source tags.\footnote{Imputation is implemented
in \texttt{impute\_missing\_cells} in \texttt{electoral/data/cleaning.py}.}
...
$P(\text{win} \mid \text{coalition})$ for the optimizer.\footnote{Implemented
as \texttt{fit\_gp\_classifier} in \texttt{electoral/models/ml\_baseline.py}.}

\subsubsection{Model specification}

We fit a \texttt{GaussianProcessRegressor} on binary $\{0,1\}$ outcomes using
an additive kernel $k = k_{\mathrm{RBF}} + k_{\text{Mat\'ern}(\nu=1.5)}$.
...
deviation) on the held-out cycle.\footnote{The LOCO-CV loop is implemented in
\texttt{electoral/models/ml\_baseline.py} and driven by
\texttt{scripts/run\_loco\_validation.py}; per-fold results are written by
\texttt{save\_loco\_json}.}
...
empirical race-bloc covariance across all cycles).\footnote{Implemented as
\texttt{estimate\_moments} in \texttt{electoral/models/ml\_baseline.py}.}
...
\textbf{Covariance} $\SigmaDelta$ is the $5\times5$ Ledoit-Wolf-shrunk covariance
\citep{Ledoit2004} of race-bloc first-differences over a restricted nine-cycle window
(1992--2024, yielding eight first-differences), not the full 1948--2024
panel:\footnote{Implemented as \texttt{build\_sigma\_delta\_from\_panel} in
\texttt{electoral/kernels/shock.py}, calling \texttt{ledoit\_wolf\_cov} in
\texttt{electoral/models/bootstrap.py}.}
...
The pre-1990 cutoff (\texttt{COVARIANCE\_MIN\_CYCLE = 1990}, applied before
first-differencing) is necessary because the data-thin minority blocs have sparse or
imputed cells in early cycles: an empty 1968 ANES Asian cell produces a spurious
$+0.71$ first-difference, and other-race 1948--1956 values are carry-forward imputed.
These artifacts inflated thin-bloc variance by more than $2\times$. Restricting to the
nine-cycle window collapses the inflated standard deviations
(Asian $0.243 \rightarrow 0.122$, other race $0.232 \rightarrow 0.096$) while leaving
well-sampled blocs essentially unchanged (White $0.077 \rightarrow 0.071$). The
resulting matrix is symmetric, positive-definite, and non-diagonal, with off-diagonal
terms capturing genuine cross-bloc co-movement. The cost is a small sample: eight
first-differences over five blocs. Shrinkage mitigates the resulting estimation error
but the covariance remains data-limited for the Asian and Latino blocs, whose corrected
standard deviations ($\approx 0.12$) sit at the upper edge of the bloc range.
...
The correct approach is Tikhonov regularization toward an equal-weight
prior:\footnote{The regularized normal-equations solve and the exact-simplex
projection are implemented in \texttt{rake\_layer\_weights} in
\texttt{electoral/kernels/raking.py}.}
...
The post-shock effective loyalty scalar is:\footnote{Implemented as
\texttt{solve\_dqcp} in \texttt{electoral/optimization/dqcp.py}.}
...
The program is solved by CVXPY; internally the quasi-convex ratio is homogenized
into a second-order cone program (SOCP) over auxiliary variables $(\mathbf{z}, s)$
with $\boldw = \mathbf{z}/s$, solved by the CLARABEL interior-point solver. The
implementation asserts disciplined quasi-convexity at runtime as a correctness
guard.\footnote{The SOCP reformulation, solver call, and \texttt{is\_dcp(qcp=True)}
runtime assertion are implemented in \texttt{electoral/optimization/dqcp.py}
(\texttt{solve\_dqcp}).}

\subsubsection{Per-bloc box constraints}
\label{sec:bounds}

The unconstrained objective in \eqref{eq:dqcp_obj} maximizes win probability over the
full simplex, which drives weight onto whichever bloc has the highest loyalty-to-variance
ratio---in the Democrat baseline, roughly $0.50$ on African Americans against a
$\approx$19\% electorate share. Such single-bloc concentrations are strategically
implausible (a coalition cannot be half African American) and inflate the win
probability by understating realized variance. We therefore constrain each weight to a
box around its National Election Pool electorate share $w_0^{(i)}$:

\begin{equation}
  w_0^{(i)} \cdot \text{LOWER} \;\leq\; w_i \;\leq\; \min\!\bigl(1,\, w_0^{(i)} \cdot \text{UPPER}\bigr),
  \qquad \text{LOWER} = 0.5,\ \text{UPPER} = 1.5,
  \label{eq:box}
\end{equation}

with $\mathbf{w}_0 = ($African American $0.12$, Latino $0.15$, Asian $0.05$, White
$0.63$, Other Race $0.05)$. The bounds are homogeneous-degree-one in $(\mathbf{z}, s)$
($z_i \geq \text{LOWER}\cdot w_0^{(i)}\, s$ and $z_i \leq \text{UPPER}\cdot w_0^{(i)}\, s$),
so they enter the SOCP as linear constraints rather than as post-hoc clipping, preserving
the quasi-convexity guarantee \citep{Lobo1998}. The multipliers are provisional pending
calibration; a shock whose required reallocation exceeds the box is correctly reported as
infeasible---a feature, not a failure.\footnote{The box bounds are constructed in
\texttt{solve\_rebalanced} in \texttt{electoral/portfolios/cvx.py} (NEP shares from
\texttt{DEFAULT\_NEP\_SHARES} in \texttt{electoral/kernels/baseline.py}) and enforced as
a \texttt{ConstraintSpec} in \texttt{electoral/portfolios/constraints.py}.}
...
constraint.''\footnote{Implemented as \texttt{solve\_baseline} in
\texttt{electoral/portfolios/cvx.py}.} When $\boldmu^\top \boldw$ already exceeds $\Veq$ for the
...
Logistic-Normal ILR draws. Let $\boldw^*$ be the optimizer
solution.\footnote{Implemented in \texttt{electoral/simulation/montecarlo.py}
(\texttt{helmert\_matrix}, \texttt{ilr}, \texttt{ilr\_inv},
\texttt{run\_ilr\_montecarlo}).}
...
Two consistency properties tie the Monte Carlo to the optimizer. First, the
simulation consumes the \emph{same} shock covariance $\SigmaDelta$ used by the
optimizer (the Ledoit-Wolf estimate of Section~\ref{sec:moments}), rather than
an isotropic diagonal fallback, so the credible interval reflects real
cross-bloc correlation. Second, the fixed-strata (religion and gender)
contribution to the win condition is derived directly from the optimizer's
reported $\tilde{\mu}_{\mathrm{eff}}(\boldw^*)$ rather than recomputed from a
neutral $\mu = 0.50$ prior; an earlier implementation held the fixed strata at
a flat neutral value, which could make the Monte Carlo's win check disagree
with the optimizer's own feasibility determination. With the current
construction, the two stages threshold the identical post-shock loyalty scalar.
...
historical shock events.\footnote{QLoRA training is implemented in
\texttt{electoral/llm/trainer.py} (\texttt{train}, \texttt{LoraConfig}),
orchestrated as a pipeline stage in \texttt{electoral/kernels/finetune.py}.}
...
\citep{Willard2023}.\footnote{Constrained and greedy decoding are implemented
as \texttt{\_predict\_constrained} and \texttt{\_predict\_greedy} in
\texttt{electoral/llm/inference.py}, orchestrated by \texttt{ShockEstimator}.}
Token midpoints span $[-0.12, +0.12]$ in vote-share
...
the four-axis taxonomy (stratum $\times$ valence $\times$ intensity $\times$
party),\footnote{Implemented in \texttt{scripts/synthetic/generate\_deepseek.py}.}
(ii) Gemini 2.5 Flash plausibility review (APPROVE / REVISE / HUMAN\_REVIEW
verdicts),\footnote{Implemented in \texttt{scripts/synthetic/gemini\_review.py}.}
and (iii) a mandatory 5\% human spot-check. Every synthetic row carries
...
(iii) pairwise correlation difference (PCD) Frobenius
norm.\footnote{The MMD/PCA/PCD diagnostics and per-row soft weights are
implemented in \texttt{scripts/generate\_synthetic.py}.} Per-row soft weights
...
Because the synthetic records are produced by external APIs whose model
versions cannot be pinned, two additional safeguards apply. First, the
committed JSONL files are the frozen artifacts: training always reads the
committed records, never a fresh API query. Second, an ablation protocol is a
paper deliverable: we report held-out MAE for (a) the base model with no
fine-tuning, (b) an adapter trained on hand-authored records only, and (c) an
adapter trained on the full augmented set. If (c) does not improve on (b), the
synthetic rows are excluded from the final dataset. As part of the residual-lean
investigation (Section~\ref{sec:model_lean}), the seed corpus was additionally
rebalanced to a $-0.0007$ party label asymmetry---effectively zero---so that
training-data composition can be ruled out as a source of directional bias.
...
\textbf{Directional validation.} The pre-augmentation adapter exhibited a
directional error: on the Newsom 2028 presidential nomination scenario it produced
\texttt{slight\_pos} for all five race blocs---a politically implausible prediction,
as Newsom is expected to hurt with White working-class and some Latino voters while
possibly helping with African American and secular blocs. The error indicated that
the model was biased toward positive bins by the synthetic training distribution
rather than learning genuinely event-specific partisan dynamics. Retraining on the
1,136-record augmented dataset, which expands coverage of events with mixed or
negative minority-bloc effects, \emph{resolves} the error. Evaluated on production
hardware (Modal A10G, version-2 adapter with seeded constrained decoding), the Newsom
scenario now differentiates correctly by bloc: White \texttt{slight\_neg}, Latino
\texttt{slight\_neg}, Evangelical \texttt{mod\_neg}, Secular \texttt{slight\_pos}. The
Latino bloc sits near a bin boundary and is the least stable across seeds; we flag it
explicitly and require at least three seeded passes under the identical adapter and
decoding path before any headline scenario is reported as the paper baseline.

\subsection{Symmetry and valence validation}
\label{sec:symmetry_methods}

Individual shock outputs are untrustworthy without a systematic check that the model
treats the two parties symmetrically and moves in the correct direction for events of
known valence. Without it, a counterintuitive-but-correct prediction cannot be
distinguished from a model that is merely pattern-matching party labels. We run two
complementary validations (\texttt{scripts/validate\_symmetry.py},
\texttt{scripts/symmetry\_test\_events.py}), both under a fixed seed and the production
constrained-decoding path.

\textbf{Symmetry test.} For each of ten matched event types (financial scandal,
endorsement, rally, debate loss, popular policy, gaffe, indictment, jobs news,
extremist remarks, convention), we author a Democrat-framed and a Republican-framed
version of the otherwise-identical shock and compare the predicted per-bloc deltas. We
report (i) the \emph{sign-mirror rate}---the fraction of blocs whose deltas flip sign
appropriately across frames; (ii) the \emph{mean magnitude ratio}
$|\delta_{\text{dem}}| / |\delta_{\text{rep}}|$, where a value far from one signals that
one party's framing moves harder; and (iii) the \emph{asymmetry}
$\overline{\delta}_{\text{dem}} - \overline{\delta}_{\text{rep}} \in [-1, 1]$, with
$|\text{asymmetry}| < 0.01$ treated as symmetric and $< 0.03$ as a mild lean.

\textbf{Valence test.} For six events with unambiguous expected direction (e.g., a
bribery indictment $\rightarrow$ negative; a record rally and polling surge
$\rightarrow$ positive), we confirm that the aggregate $\Delta_{\mathrm{eff}}$ moves
with the expected sign. A separate end-to-end harness
(\texttt{scripts/verify\_pipeline.py}) checks the covariance properties of
Section~\ref{sec:moments}, the optimizer box bounds of Section~\ref{sec:bounds}, and
full LLM~$\rightarrow$~optimizer~$\rightarrow$~Monte Carlo runs for absence of NaN/Inf
and well-formed credible intervals; the latest run passes 16 of 17 checks (the single
skip is a tokenizer-instantiation guard on the offline test host). Results are reported
in Section~\ref{sec:symmetry_results}.
...
few-shot classifier \citep{Tunstall2022} on bio text,\footnote{Implemented as
\texttt{BioClassifier} in \texttt{electoral/nlp/bio\_classifier.py}, served by
\texttt{scripts/pi\_bio\_server.py}.} served from a
...
sentence encoder \citep{Liu2019}.\footnote{Implemented as \texttt{RoBERTaScorer}
in \texttt{electoral/nlp/scorer.py}.} The resulting ElasticityScore scalars
$\in [-1, 1]$ serve as input features to the Stage~1 LLM alongside the
free-text shock description.\footnote{Implemented as \texttt{estimate\_elasticity}
/ \texttt{fit\_elasticity} in \texttt{electoral/nlp/elasticity.py}.}
...
structural threshold. We note that the win-probability saturation observed under
smoke-configuration conditions ($P(\text{win}) \approx 1.0$) was initially
attributed in part to this threshold being too low, but was subsequently
root-caused to the near-zero diagonal covariance fallback alone
(Section~\ref{sec:saturation}); the threshold question stands on its own merits.
This value will be validated and potentially revised in consultation with the
supervising faculty (Section~\ref{sec:conclusion}).
\end{remark}
...
\textbf{Platt scaling.}\footnote{Implemented as \texttt{platt\_scale\_loco} in
\texttt{electoral/models/ml\_baseline.py}.} We implemented jackknife Platt
scaling as a post-hoc calibration step and found that it degraded every metric (accuracy: 0.800
...
\subsection{Bipartisan neutrality and symmetry validation}
\label{sec:symmetry_results}

\textbf{Zero-shock diagnostic.} A run with all deltas set to zero is the cleanest
probe of structural bias: with no shock signal, the LLM contributes nothing, so the
two parties' baseline win probabilities should be near-symmetric around the
EC-adjusted target. They were not. The original model gave Democrats a baseline win
probability of $1.00$ against $0.26$ for Republicans---an asymmetry baked into the
\emph{scoring}, independent of any prediction.

\textbf{Root cause and fix.} The fixed-loyalty term contributed by religion and gender
(Section~\ref{sec:mueff}) used equal within-stratum weights ($v_r = 1/7$, $g_g = 1/3$).
With seven religion and three gender blocs, this assigned $\approx 0.14$ to each
religion bloc and $\approx 0.33$ to each gender bloc, wildly over-weighting small but
strongly Democratic groups (Jewish $\approx 2\%$, Muslim $\approx 1\%$, other gender
$\approx 1\%$ of the electorate). The inflated Democratic religion/gender mean
propagated into $\mueff$ as a constant Democratic tailwind on every scenario. Replacing
the $1/n$ placeholder with documented electorate marginals
(Section~\ref{sec:mueff}; Pew/ANES for religion, CPS/ANES for gender) reduced the
baseline asymmetry by 98\%, from $+0.052$ to $+0.001$. The small residual favors
Republicans, which is intended: the EC-adjusted target
($\Veq = 0.5066$ Democrat, $0.4934$ Republican) deliberately encodes the Democrats'
structural Electoral College disadvantage.

\textbf{Residual win-probability asymmetry is a property, not a bias.} After the fix,
the baseline means are symmetric (Democrat margin $+0.025$, Republican $+0.032$), yet a
win-probability gap persists at zero shock (Democrat $\approx 0.98$, Republican
$\approx 0.46$). This is not mean bias---both parties have small positive margins.
It arises because the Republican optimal coalition is structurally \emph{more
concentrated}: with minority blocs near their lower box bounds and White's large NEP
share ($0.63$), the Republican optimum places $\approx 82\%$ of weight on White,
producing higher Monte Carlo variance and thus a lower probability of clearing the
threshold \emph{despite a larger mean margin}. A counterfactual single-bloc cap
(White $\leq 0.792$) raises the Republican win probability to $\approx 0.52$, confirming
that the mechanism is coalition concentration, not baseline asymmetry. We retain this as
a modeled property rather than tuning it away: a single-bloc-dependent coalition is
genuinely more electorally fragile, and engineering win-probability symmetry would mask
a real structural difference between the parties' coalitions. Reported results therefore
lead with the (symmetric) margin and present win probability with this variance caveat.

\textbf{Directional symmetry.} The matched-pair and valence tests of
Section~\ref{sec:symmetry_methods} confirm that, after the fix, the model treats
Democrat- and Republican-framed versions of the same event symmetrically (signs mirror
across frames; magnitude ratios near one) and that known-valence events move in the
expected aggregate direction. This structural-scoring correctness is distinct from, and
a precondition for, the directional accuracy of the Stage~1 predictions themselves
(Section~\ref{sec:llm}).

\textbf{Residual behavioral lean.} With the structural baseline leveled, the
matched-pair test isolates a remaining $+0.028$ Democratic lean in the Stage~1
model's \emph{predictions} (mean asymmetry
$\overline{\delta}_{\text{dem}} - \overline{\delta}_{\text{rep}}$ across matched
event pairs). Three candidate causes were tested by elimination. (i)
\emph{Baseline aggregation bias}: corrected by the electorate-marginal fix above;
the lean persisted. (ii) \emph{Covariance artifacts}: corrected by threading the
real Ledoit-Wolf $\SigmaDelta$ through the Monte Carlo (Section~\ref{sec:mc});
not the source. (iii) \emph{Synthetic corpus label imbalance}: the seed corpus was
rebalanced to a $-0.0007$ label asymmetry and the adapter retrained; the lean
persisted at $+0.028$, ruling out training-data composition. With all three
eliminated, the residual localizes to the base model's pretrained political
priors, and it concentrates in \emph{valence-interpretation} events---a
Republican gaffe is read as more damaging than the Democratic equivalent, and
positive economic news as more Democrat-helping. Section~\ref{sec:model_lean}
discusses why we disclose this rather than correct it post hoc.

% -----------------------------------------------------------------------
\section{Deployed System}
\label{sec:system}

The full three-stage pipeline is deployed as a live, public-facing web application.
The intent is twofold: to demonstrate that the framework runs end-to-end on
free-text input in interactive time, and to provide the supervising faculty an
inspectable interface over the model's intermediate artifacts.

\subsection{Streaming inference service}

The backend is a FastAPI application served on Modal using a single A10G GPU
(24\,GB).\footnote{The FastAPI application is implemented in
\texttt{electoral/api/shock\_endpoint.py}; the Modal container/GPU configuration
is defined in \texttt{deploy/modal\_app.py}.}
At container start the Mistral 7B base model is loaded in \texttt{float16} ($\approx
14$\,GB) and the rank-16 version-2 QLoRA adapter ($\approx 52$\,MB) is merged. The
primary endpoint, \texttt{GET /estimate/stream}, accepts a shock description, an
intensity in $[0.1, 3.0]$, and a party, and returns Server-Sent Events as each pipeline
stage completes: a \texttt{deltas} event (Stage~1 LLM bins, $\approx 2$\,s), an
\texttt{equilibrium} event (Stage~2 DQCP coalition weights and feasibility,
$\approx 1$\,s), a \texttt{simulation} event (the 10{,}000-draw Logistic-Normal ILR
Monte Carlo with its credible interval, $\approx 0.5$\,s), and a terminating
\texttt{done} event. The three stages run in thread/process pools (the CVXPY solve is
isolated in a process pool because it is not thread-safe). Each request is written to a
DuckDB audit log with per-stage latencies.\footnote{Implemented as
\texttt{AuditLogger} in \texttt{electoral/api/audit.py}.} For cost control the service
scales to zero
($\texttt{min\_containers} = 0$, $60$\,s scaledown), giving a $\approx 24$--$30$\,s cold
start and $\approx 5$--$15$\,s warm round-trips.

\subsection{Web interface}

The frontend is a Next.js application (Tailwind CSS, Recharts) deployed on Vercel and
pointed at the Modal endpoint.\footnote{Top-level state machine and SSE progressive
reveal implemented in \texttt{webapp/app/page.tsx}.} A user selects a party, types a
shock, and sets an
intensity (or picks one of five preset event categories); the results then populate
progressively as the SSE stream arrives. Because many shocks are
incumbent-relative---a recession or an administration scandal reads differently
depending on which party holds power---the interface also offers a ruling-party
selector. Since the Stage~1 model consumes free text rather than a structured
governing-party field, the selection is woven into the event description itself
as a natural-language clause (``\ldots under the current Republican
administration''), skipped when the text already establishes governing context,
with a preview of the exact string sent to the model.\footnote{Implemented as
\texttt{injectRulingContext} in \texttt{webapp/lib/rulingParty.ts}, with the
selector UI in \texttt{webapp/components/ShockInput.tsx}.} Three widgets render the output:
(i) a plain-language \emph{narrative} that groups blocs into gaining, losing, and---
distinct from earlier versions---little-measurable-effect (neutral)
buckets;\footnote{Implemented in \texttt{webapp/components/ShockNarrative.tsx}.}
(ii) a two-panel \emph{coalition chart} that deliberately separates the predicted
within-bloc loyalty shift $\tilde\mu_i$ from the optimizer-recommended coalition
emphasis $\tilde w_i$, relabeling the latter as strategic weighting rather than
population share to prevent a common misreading;\footnote{Implemented in
\texttt{webapp/components/CoalitionChart.tsx}.} and (iii) a \emph{win-probability
gauge} with a percentile strip showing the 90\% credible interval.\footnote{Implemented
in \texttt{webapp/components/WinGauge.tsx} and
\texttt{webapp/components/PercentileStrip.tsx}.} A share button
encodes the query into URL parameters.\footnote{Implemented as \texttt{handleShare}
in \texttt{webapp/app/page.tsx}.} An authenticated analyst dashboard exposes six
panels over the pipeline internals: a data-coverage matrix, the RoBERTa sentiment
distribution, bio-classifier method coverage, training-loss curves, Monte Carlo
convergence, and a searchable audit table.\footnote{Assembled in
\texttt{webapp/app/dashboard/page.tsx} from
\texttt{webapp/components/dashboard/\{CoverageMatrix,SentimentDistribution,
BioCoverage,LossCurves,Convergence,AuditTable\}.tsx}.}

\subsection{Live-demo caveat}

The deployed demo prioritizes responsiveness and cost; for paper-grade numbers the
production-grade covariance path (Ledoit-Wolf $\SigmaDelta$ of
Section~\ref{sec:moments}) must be loaded into the serving image rather than the
diagonal fallback, and the market-comparison module was removed because it juxtaposed
the model's conditional $P(\text{win}\mid\text{shock})$ against a market's unconditional
price (Section~\ref{sec:discussion}). We also note that the public share-link feature
should be rate-limited before wide release, since each call triggers a GPU invocation.
...
A related caveat concerns the win condition itself. The optimizer thresholds
$\mueff$---a $\boldsymbol{\lambda}$-weighted blend over three
\emph{overlapping} strata---against $\Veq$, which is defined on the national
two-party popular-vote share. The two quantities coincide only under the
additive independence approximation: each stratum independently covers the
electorate, so each stratum average is separately an estimate of the national
share, and a convex combination of the three inherits that interpretation
exactly when the approximation holds. Where intersectional interactions are
material, $\mueff$ is a biased estimator of the popular-vote share and the
comparison to $\Veq$ inherits that bias. We state this explicitly as a
limitation; the additive-versus-raked comparison above is the empirical check
on its severity.

\subsection{Residual model lean from pretrained priors}
\label{sec:model_lean}

The $+0.028$ Democratic lean documented in Section~\ref{sec:symmetry_results}
is a property of the Stage~1 model's predictions, not of the scoring pipeline.
The elimination analysis is what licenses this attribution: with the baseline
aggregation corrected, the covariance path corrected, and the training corpus
rebalanced to a $-0.0007$ label asymmetry---after which the lean persisted
unchanged---the only remaining candidate is the base model's pretrained
political priors. This is consistent with the mechanics of low-rank
adaptation: a rank-16 LoRA adapter re-weights the base model's behavior at the
margin but does not overwrite priors encoded in the frozen weights
\citep{Hu2022}. The lean concentrates in valence-interpretation events, where
the model must judge \emph{how damaging} or \emph{how helpful} an event is
rather than merely which blocs it touches.

We retain and disclose the lean rather than engineering it out. An output-side
correction tuned to zero the asymmetry on the tested event set would hide a
real property of the underlying model without fixing it, and there is no
guarantee it would generalize beyond that set. Users of the framework should
read Democrat-favorable predictions on valence-ambiguous events with a
$\approx 3$-point discount in delta-bin units. Three escalating mitigations
are available to future work: heavier fine-tuning (higher LoRA rank, more
epochs, or full-parameter tuning) to overwrite more of the pretrained priors;
base-model selection across less politically leaning open-weight checkpoints;
or a documented output-side calibration applied symmetrically to both parties
...
and PredictIt contract prices.\footnote{Implemented as \texttt{MarketCollector}
in \texttt{electoral/markets/collector.py} and \texttt{MarketAggregator} in
\texttt{electoral/markets/aggregator.py}; contract IDs are configured in
\texttt{configs/market\_contracts.json}.} We use prediction markets as post-hoc
...
\boldw}$ is dominated by the near-zero denominator and diverges.

The root cause is the covariance source, not the threshold. When real panel
data is absent, the diagonal fallback $\SigmaDelta = 0.02^2 I$ produces
negligible denominator variance, and \emph{any} positive margin---however
small---maps to a saturated win probability. An earlier diagnosis attributed
the saturation partly to $\Veq = 0.5066$ being too low
(Section~\ref{sec:veq_results}); subsequent testing with the Ledoit-Wolf
$\SigmaDelta$ threaded through the full pipeline showed that the saturation
disappears once the covariance is non-degenerate, regardless of the threshold
value. A lower threshold enlarges the margin and thus amplifies the effect,
but does not cause it. Production runs use the Ledoit-Wolf estimator on the
nine-cycle (1992--2024) race-delta matrix (Section~\ref{sec:moments}), which
yields non-degenerate off-diagonal terms; the diagonal fallback is retained
only for smoke configurations and, currently, the cost-constrained live demo.
A variance floor on the supplied $\SigmaDelta$ additionally prevents a
near-zero empirical covariance from collapsing the credible interval to a
degenerate $[1,1]$ or $[0,0]$.

Results are interpretable once the real covariance is loaded; validating
$\Veq$ against EC efficiency models remains a separate open item on its own
merits.
...
The three-stage pipeline is implemented and deployed end-to-end. The Stage~1 LLM
(Mistral 7B QLoRA rank-16) is fine-tuned and evaluated, and its earlier directional
error is resolved on the retrained adapter; the Stage~2 DQCP optimizer solves on real
panel data under per-bloc box constraints; the Stage~3 NLP pipeline (SetFit
bio-classifiers and RoBERTa scorer) is trained and deployed. The bipartisan-neutrality
fix removes the structural Democratic scoring bias, and the whole pipeline is served as
a live streaming web application (Section~\ref{sec:system}). The shock catalog covers
30 historical events (2001--2024) across nine taxonomy categories.
...
\begin{enumerate}
  \item \textbf{Threshold validation.} $\Veq = 0.5066$ must be confirmed or
    ...
    the true Democrat Electoral College structural disadvantage. The
    win-probability saturation once attributed to this value has been
    root-caused to the degenerate covariance fallback instead
    (Section~\ref{sec:saturation}); the threshold question remains open on
    its own merits.

  \item \textbf{Production covariance in the live demo.} The deployed service must
    load the nine-cycle Ledoit-Wolf $\SigmaDelta$ in place of the diagonal fallback,
    and the headline scenarios must be reported over at least three seeded passes
    (noting the Latino bloc's boundary instability), before the live outputs match
    the paper baseline.
...
The directional validation of the Stage~1 model and the bipartisan-neutrality
requirement, both previously open, are now satisfied (Sections~\ref{sec:llm}
and~\ref{sec:symmetry_results}). The remaining $+0.028$ Democratic lean in the
model's predictions is root-caused to the base model's pretrained priors and
disclosed as a limitation with a quantified magnitude and localization
(Section~\ref{sec:model_lean}), rather than masked by an output-side
correction. The current within-stratum marginals are documented
electorate shares, an interim resolution of the layer-weight calibration; replacing them
with full \texttt{raking.py} iterative proportional fitting against the live panel
remains a longer-term refinement.
...
gender cross-tabulations; (iv) neutralization of the pretrained-prior lean via
heavier fine-tuning or base-model selection (Section~\ref{sec:model_lean});
and (v) post-election 2026--2028 validation against observed party strategic
responses to the Iran conflict and the 2026 midterms.
...
implementation for correctness in the domain-agnostic case.\footnote{Implemented
as \texttt{project\_simplex} / \texttt{project\_simplex\_batch} in
\texttt{electoral/optimization/simplex.py}.}
...
\bibitem[Lobo et~al.(1998)]{Lobo1998}
Lobo, M.~S., L. Vandenberghe, S. Boyd, and H. Lebret (1998).
\newblock Applications of second-order cone programming.
\newblock \textit{Linear Algebra and its Applications}, 284(1--3), 193--228.
