...
bipartisan-neutrality requirement. The race-bloc covariance $\SigmaDelta$ is
...
\begin{enumerate}
  ...
  \item A structural correctness result satisfying a bipartisan-neutrality
        ...
        Democrat/Republican event pairs and known-valence events.
...
progressively as the SSE stream arrives. Three widgets render the output:
...
\begin{enumerate}
  \item \textbf{Covariance source.} When real panel data is absent, the diagonal
    ...
    Production runs use the Ledoit-Wolf estimator on the nine-cycle (1992--2024)
    race-delta matrix (Section~\ref{sec:moments}), which yields non-degenerate
    off-diagonal terms; the diagonal fallback is retained only for smoke
    configurations and, currently, the cost-constrained live demo.
...
\begin{enumerate}
  \item \textbf{Threshold validation.} $\Veq = 0.5066$ must be confirmed or
    ...
    value produces win-probability saturation under degenerate-covariance
    conditions, which limits the optimizer's diagnostic value.
...
and~\ref{sec:symmetry_results}). The current within-stratum marginals are documented
