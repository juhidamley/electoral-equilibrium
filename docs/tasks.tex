...
\begin{taskbox}
\task{Schedule and hold an overdue supervision check-in with Prof.\
  Espinosa (last check-in was Week~2; major methodology has changed
  unreviewed since). Walk in with a written agenda, priority-ordered:
  (1) the synthetic data augmentation pipeline
  (DeepSeek$\to$Gemini$\to$human review, 559 LLM-generated records
  merged into the 1,136-record training set) --- is training a
  forecaster on LLM-labeled data methodologically defensible, and how
  must it be described/caveated in the methods section? Raise the
  647-record unreviewed queue and the shared-blind-spot risk yourself;
  (2) the definition of \texttt{target}=0.5066 (drives win\_prob
  saturation);
  (3) the win\_probability CI meaning (sampling error vs
  $\Sigma_\Delta$-propagated uncertainty);
  (4) ratify the NEP-weighted baseline change
  ($\mu_\text{eff}$ 0.716$\to$0.605);
  (5) confirm the model-vs-market framing.
  Capture every answer in \texttt{DECISIONS.md} and update the
  corresponding flag-log items to ``resolved/deferred per Espinosa
  \textit{<date>}''. Several correctness flags CANNOT close without this
  meeting --- it is a prerequisite, not a formality. Severity: blocking
  for the SRP deliverable.}{3}

\task{PROVISIONAL pending Espinosa check-in (last check-in Week~2).
  Working assumptions used to keep development unblocked --- each MUST be
  confirmed or corrected at the meeting; do NOT treat as settled:
  \begin{itemize}[leftmargin=1.6em, itemsep=1pt]
    \item \texttt{target=0.5066} assumed to mean national two-party
      popular-vote share needed for an EC win. [PROVISIONAL --- likely too
      low; Dems need $\sim$0.52]
    \item win\_probability CI assumed to be Monte Carlo sampling error,
      explicitly NOT covariance-propagated. [PROVISIONAL --- methods
      section must state whichever he confirms]
    \item Synthetic-data training accepted as-is pending his ruling on
      whether LLM-labeled data is methodologically admissible.
      [PROVISIONAL --- highest-risk assumption]
  \end{itemize}
  Every number/figure produced under these is marked PROVISIONAL in
  \texttt{DECISIONS.md} until the meeting. The flag-log items stay OPEN
  --- these defaults do not close them.}{2}
...
\task{Flag-log entries captured so far (append below as new ones arise):
  \begin{itemize}[leftmargin=1.6em, itemsep=1pt]
    ...
    \item[$\boxtimes$] \textbf{[blocking --- RESOLVED 2026-06-24 LIVE]}
      Newsom directional error confirmed fixed on production Modal A10 GPU
      with v2 adapter + seeded constrained decoding:
      white=slight\_neg, latino=slight\_neg, evangelical=mod\_neg,
      secular=slight\_pos. Direction differentiates correctly by bloc.
      Latino is no longer boundary-unstable under the seeded path.
      MVP directional validation PASSED. Paper-grade re-check needed
      to confirm these exact values are stable across $\geq$3 seeded runs
      before citing in a results table.
    ...
    \item[$\square$] \textbf{[correctness --- CONFIRMED OPEN 2026-06-24]}
      Production covariance is the identity/diagonal fallback, NOT
      Ledoit-Wolf. Confirmed live: win-prob CI [0.653, 0.668] is MC
      sampling error around diagonal cov. OK for MVP; blocks paper-grade
      results. Fix: load real panel data so
      \texttt{run\_ilr\_montecarlo} uses the 20-cycle Ledoit-Wolf
      $\Sigma_\Delta$ rather than the near-zero fallback.
    ...
    \item[$\square$] \textbf{[hardening $\to$ possibly blocking]} 647-record
      Gemini human-review queue is EXCLUDED from the current 1,136-record
      training set (577 original + 559 Gemini-approved) --- cautious
      default. Gate: validate Newsom direction on the 1,136-set model.
      If corrected $\Rightarrow$ queue is quality-improvement only
      (adjudicate when convenient). If NOT corrected $\Rightarrow$ queue
      is blocking --- some held-out records may be required for
      directional correctness and must be adjudicated and merged before
      the model is paper-ready.
    ...
    \item[$\square$] \textbf{[correctness]} Modal audit DuckDB is
      ephemeral --- container restarts silently drop all audit rows.
      Mount on a Modal Volume or external DB; confirm with a
      deploy-restart-query test.
    \item[$\square$] \textbf{[correctness --- Espinosa-grade]} Bias/symmetry
      of the model across parties not yet formally validated. Matched
      scandal pairs show: race deltas near-symmetric (Rep aa $-$0.07 vs
      Dem $-$0.035; remaining blocs equal); religion deltas differ by
      coalition shape (Dem scandal: evangelical/prot $+$0.012; Rep scandal:
      evangelical/catholic $-$0.07) --- signs are coherent (scandal hits
      own base hardest), not obvious anti-R bias. BUT: \texttt{delta\_eff}
      inverts oddly (Dem scandal $\to$ Dem $\Delta_\text{eff}=-0.0$; Rep
      scandal $\to$ Rep $\Delta_\text{eff}=+0.03$) --- possible per-party
      aggregate calibration bug that could drive win-prob asymmetry.
      Required: run 5--10 matched event-type pairs, measure symmetry
      formally (e.g.\ sign-parity and magnitude ratio across parties)
      before any claim that the model is ``bipartisan.'' This is a
      methodology question for the Espinosa check-in --- a model presented
      as party-neutral must demonstrate it empirically.
    \item[$\square$] \textbf{[blocking --- PUBLIC DEPLOY + LIVE COST]}
      \texttt{min\_containers=1} is deployed (warm A10, fixes CORS/150s
      timeout root cause) but costs \textasciitilde\$1/hr 24/7
      \emph{right now}. Must: (a) set Modal billing hard limit immediately;
      (b) add per-IP rate limit and concurrency cap before sharing the URL;
      (c) scale to \texttt{min\_containers=0} after the demo window closes.
      This is no longer hypothetical --- it is live spend.
      \textbf{Background:} Modal enforces a 150\,s web-request timeout;
      requests over it get a 303 redirect that strips CORS headers. Cold
      start triggered a 14\,GB model re-download (\texttt{snapshot\_download}
      used \texttt{local\_dir} not HF cache format, so \texttt{from\_pretrained}
      re-fetched every time, $\sim$3\,min). Fixed: correct \texttt{cache\_dir}
      format + \texttt{min\_containers=1}.
    \item[$\square$] \textbf{[correctness --- CONFIRMED OPEN 2026-06-24]}
      win\_probability CI [0.653, 0.668] observed live --- a 1.5\,pp
      width that is MC sampling error around the diagonal fallback cov,
      not covariance-propagated uncertainty. Will widen meaningfully once
      Ledoit-Wolf $\Sigma_\Delta$ is active (ties to covariance-source
      flag above). Decide with Espinosa: (a) Wishart-propagated outer
      loop, or (b) explicitly redefine as sampling-error bounds and
      document scope. Do not present this CI as the methodology-section
      interval until the covariance source is fixed and the semantic
      choice is documented.
    \item[$\square$] \textbf{[correctness --- paper deliverable]} Synthetic
      data ablation study not tracked: DECISIONS.md requires training two
      models --- hand-authored-only vs augmented --- and reporting both MAEs
      before the synthetic rows may be claimed as beneficial in the paper.
      If the augmented model does not improve on hand-authored-only, the
      synthetic rows must be dropped. This is a required paper section, not
      optional.
    \item[$\square$] \textbf{[correctness]} \texttt{bin\_uncertainty.json}
      $\sigma_b$ values are still unpopulated (open item in DECISIONS.md).
      The Logistic-Normal MC uses per-bin empirical $\sigma_b$; if this
      file holds defaults/zeros the CI width is wrong. Run
      \texttt{generate\_synthetic.py} to populate before the
      paper-baseline run.
    \item[$\square$] \textbf{[correctness --- paper claim]} LIWC
      cross-validation of the RoBERTa scorer is blocked (no tweet-ID
      join column in the SAV file). DECISIONS.md defers the alternative
      (100-post Spearman) to post-SRP. The methodology section must NOT
      claim LIWC validation occurred; it must accurately describe what
      validation was done (score distribution variance check only).
    \item[$\square$] \textbf{[correctness/legal]} Foster \& Rathlin
      \texttt{metoo\_liwc} dataset is CC~BY-NC-SA~4.0 (non-commercial
      only) --- flagged in DECISIONS.md for startup/commercial review.
      Confirm the SRP publication, any web demo, and any downstream use
      does not violate the non-commercial clause before submission.
    \item[$\square$] \textbf{[correctness]} NEP-weighted baseline change
      ($\mu_\text{eff}$ 0.716$\to$0.605) is listed in the Espinosa
      meeting agenda (item 4) as ``ratify'' but does not appear as a
      decided entry in DECISIONS.md. Either document it as an
      implemented decision or confirm with Espinosa that it is correct
      and then document it.
    \item[$\boxtimes$] \textbf{[blocking --- deploy --- RESOLVED 2026-06-23]}
      Constrained decoding fixed end-to-end: \texttt{pydantic.create\_model}
      + str-Enum + \texttt{outlines.models.Transformers} wrap +
      \texttt{use\_cache=False} + Enum \texttt{.value} unwrap. No fallback.
      Schema guarantee restored. Deploy blocker cleared.
    \item[$\boxtimes$] \textbf{[correctness --- paper --- RESOLVED 2026-06-23]}
      Decode non-determinism closed. Generation seeded (\texttt{seed} param
      threaded through \texttt{predict\_delta\_bins} $\to$
      \texttt{\_predict\_constrained}, default 42,
      \texttt{torch.manual\_seed} + rng). Confirmed reproducible: identical
      output across two runs. Inference path fully working: trained adapter +
      constrained decoding + seeded + clean str tokens.
      \textbf{All MVP code blockers closed.}
      Remaining paper obligation: headline-event validation tables must be
      produced under the seeded constrained path; boundary-straddling blocs
      (latino on Newsom) must be noted in the paper as carrying uncertainty
      the MAE metric does not capture.
    \item[$\square$] \textbf{[correctness]} Python 3.10+ union syntax
      (\texttt{X | None}, \texttt{X | Y}) crashes at import on local
      3.9 dev env --- already broke FastAPI server boot
      (\texttt{get\_audit}, \texttt{str | None}). Choose one fix:
      (a) convert to \texttt{typing.Optional}/\texttt{Union} throughout
      \texttt{electoral/}; (b) add \texttt{from \_\_future\_\_ import
      annotations} to every affected module (verify no runtime annotation
      introspection); or (c) bump local dev to 3.11 to match
      HPC/paper-baseline target. Clean-checkout test on 3.9 will keep
      surfacing these until resolved.
    \item[$\square$] \textbf{[blocking --- deliverable]} Methodology and
      Results sections are scaffolded, not written --- 39
      TODO/PROVISIONAL placeholders, each tied to a real output
      (Espinosa decision, paper\_baseline run, or model validation). None
      may be filled from memory or assumption. Submission gate:
      \texttt{grep -c 'TODO\textbackslash{}|PROVISIONAL'} on the paper
      must return 0. Load-bearing holes: contested $V_{eq}$ (0.535
      vs 0.5066), synthetic-data admissibility, Newsom directional
      validation.
    \item[$\square$] \textbf{[blocking --- deliverable]} Do NOT tag
      \texttt{v1.0-paper-baseline} until: (a) retrained adapter validated
      to fix the Newsom all-minorities-positive error; (b)
      Espinosa-confirmed (or explicitly-provisional) \texttt{target} and
      CI definitions; (c) green clean-checkout test suite; (d) production
      covariance confirmed as Ledoit-Wolf, not the diagonal fallback.
      Tagging early freezes known-wrong numbers as immutable ground truth.
    \item[$\square$] \textbf{[correctness/security --- blocking for public
      deploy]} Dashboard page auth (DASHBOARD\_PASSWORD) does not protect
      the FastAPI backend routes (\texttt{/api/coverage},
      \texttt{/api/sentiment-dist}, \texttt{/api/bio-coverage},
      \texttt{/api/audit}) on the Modal URL --- anyone can call them
      directly and read panel data and the full audit log. Add a shared
      bearer token / signed header checked at the FastAPI boundary before
      going public.
    \item[$\square$] \textbf{[security --- blocking for public deploy]}
      \texttt{/api/audit} search term is interpolated into SQL
      (\texttt{LIKE \%search\%} via f-string/concatenation) --- SQL
      injection on a public endpoint hitting the audit store. Use a
      DuckDB placeholder (\texttt{ILIKE ?} with wildcards in the bound
      parameter). Clamp \texttt{limit}. Audit every other DuckDB query
      for the same pattern.
    \item[$\boxtimes$] \textbf{[correctness --- RESOLVED 2026-06-29]}
      CoalitionChart rendered only the \texttt{white} bloc (4 of 5 bars
      invisible). Root cause: Recharts~3 with \texttt{layout="vertical"}
      sets \texttt{props.width} to 0 during initial render for most bars;
      the old guard \texttt{if (width <= 0) return <g/>} silently dropped
      them. Fix: derive \texttt{plotW} from \texttt{background.width}
      (\texttt{offset.width}, always the full chart-plot pixel span) and
      compute \texttt{barW = sv * plotW}. All five race-bloc bars plus
      the translucent weight overlay now render correctly. TypeScript
      type-check and production build both pass. Confirmed in
      \texttt{webapp/components/CoalitionChart.tsx}.
    \item[$\square$] \textbf{[polish]} Remove the \texttt{console.log("[BarShape]", ...)}
      debug statement in \texttt{CoalitionChart.tsx} (gated by
      \texttt{NODE\_ENV === "development"} but still noise). Remove once
      user confirms all five blocs render correctly in the browser.
    \item[$\square$] \textbf{[correctness --- optimizer]} Coalition weights
      $\tilde{w}$ were demographically unconstrained: the optimizer
      concentrated weight on the highest-loyalty bloc
      (\texttt{african\_american} $\tilde{w}\approx0.50$ vs
      $\sim$0.19 electorate share), producing unrealistic coalitions and
      inflating win probabilities. FIX IN PROGRESS: added per-bloc bounds
      $w_0[i]\!\cdot\!\texttt{LOWER\_MULT} \le w[i] \le
      w_0[i]\!\cdot\!\texttt{UPPER\_MULT}$ (provisional
      $0.5\times$--$1.5\times$ of NEP baseline share in
      \texttt{electoral/optimization/cvx.py}). Bounds are PROVISIONAL ---
      calibration is an Espinosa decision. Verify some shocks now
      correctly go infeasible (a feature, not a bug).
    \item[$\square$] \textbf{[correctness --- Espinosa]} Win-probability
      inflation: with unconstrained weights essentially all events cleared
      target (e.g.\ ``MET +16\,pp'', win-prob 0.66 for a scandal). Confirm
      after the weight-bound fix that win-probs attenuate to realistic
      levels. Ties to the open \texttt{target}=0.5066-meaning and
      diagonal-covariance flags.
    \item[$\square$] \textbf{[correctness --- simulation]} Per-bloc weight
      percentile (p5--p95) panel renders degenerate: percentiles span
      $\sim$0--0.88 with mass piled near 0, an artifact of
      (a)~diagonal-covariance fallback and (b)~unconstrained optimizer.
      Recheck after weight-bound fix; if still degenerate, either wire
      real Ledoit-Wolf $\Sigma_\Delta$ or remove the panel for the
      presentation and note as future work.
    \item[$\square$] \textbf{[correctness --- display]} ShockNarrative has
      no \texttt{neutral} bucket: blocs with
      \texttt{delta\_bin=="neutral"} are sorted into hurt/help, making
      output appear more polarized than the model's actual predictions
      (which include many neutrals). Add a third ``little measurable
      effect on:'' bucket.
    \item[$\square$] \textbf{[polish --- display]} CoalitionChart
      conflated two distinct quantities ($\tilde\mu$ loyalty and
      $\tilde{w}$ coalition emphasis) in one overlay; users misread
      $\tilde{w}$ as population share. FIX IN PROGRESS: split into two
      panels (loyalty shift / coalition emphasis) + add an explainer panel
      defining each term + relabel $\tilde{w}$ as ``coalition emphasis
      (strategic weighting), not population share.''
    \item[$\square$] \textbf{[polish --- display]} Recharts~v3 passes
      \texttt{width=0} to custom bar shape during initial layout, tripping
      the \texttt{width<=0} guard; only one bloc's bar was visible.
      Partial fix via \texttt{background.width} (see RESOLVED entry
      above). Splitting into two single-quantity panels sidesteps the
      fragile overlay entirely; verify all 5 blocs render in both panels
      post-split.
    \item[$\boxtimes$] \textbf{[polish --- DONE]} Removed market-prior /
      model-vs-market module (``No active market'',
      Polymarket/Manifold comparison): almost always empty, and when
      populated compared the model's conditional
      $P(\text{win}\mid\text{shock})$ against the market's unconditional
      $P(\text{win}\mid\text{current})$ --- different questions, misleading
      juxtaposition. Removing it also eliminated
      \texttt{/api/market-prior} CORS errors.
    \item[$\square$] \textbf{[hardening --- deploy/cost]}
      Modal cost control: \texttt{min\_containers=0},
      \texttt{scaledown\_window=60}, \texttt{cache\_dir} baked into the
      image (cold start 23.7\,s, no 14\,GB re-download). HARD CONSTRAINT:
      keep Modal under \$15/month. NEVER set \texttt{min\_containers>0}
      except during an active demo window, then revert. Manual task: set
      Modal dashboard billing cap. Run all batch/validation work on
      Hopper (free); Modal only for the live demo URL.
    \item[$\square$] \textbf{[correctness --- validation --- HIGH PRIORITY]}
      No systematic symmetry/valence validation has been run. Individual
      outputs are untrustworthy without it (cannot distinguish
      ``counterintuitive-but-correct'' from ``biased or
      pattern-matching party labels''). REQUIRED before presentation:
      (1)~\emph{symmetry test} --- matched event pairs across event types,
      measure whether the two parties are treated symmetrically;
      (2)~\emph{valence test} --- events with known unambiguous direction,
      confirm aggregate $\Delta_\text{eff}$ moves correctly. Run on
      Hopper (free). Required paper/presentation deliverable regardless of
      outcome.
    \item[$\boxtimes$] \textbf{[decision --- REJECTED]} Considered
      (a)~migrating hosting from Modal to Hopper and (b)~using Grok~2 as
      the base model or API output-corrector. Both REJECTED: Hopper cannot
      host a persistent public URL (job scheduler, no inbound routing);
      Grok~2 is 500\,GB/8-GPU (won't fit), and its Community License
      prohibits using it to train or improve other AI models, invalidating
      the synthetic-data and corrector use cases.
      Grok-as-EVALUATOR (score outputs, don't alter them) is acceptable;
      Grok-as-CORRECTOR is not (unreproducible, replaces the model's own
      judgment, license violation).
    \item[$\square$] \textbf{[context --- one week to presentation]}
      Priority order: (1)~optimizer weight-bound correctness fix;
      (2)~symmetry/valence validation on Hopper;
      (3)~display fixes (neutral bucket, two-panel split, explainer);
      (4)~overdue Espinosa meeting on \texttt{target} meaning +
      covariance + weight-bound calibration.
  \end{itemize}
...
\task{Persist the audit DuckDB across Modal container restarts.
  \texttt{data/audit.duckdb} is a local file; Modal's container filesystem
  is ephemeral, so audit rows are silently lost on every cold start /
  redeploy. Mount the DuckDB file on a Modal Volume (or move audit storage
  to an external persistent DB). Until then the audit log works locally
  but is decorative in production --- confirm persistence with a
  deploy-restart-query test (log an estimate, force a container restart,
  confirm the row survives). Severity: correctness --- an audit trail that
  doesn't persist is not an audit trail.}{2}
\end{taskbox}
...
\task{DONE (MVP): Validate the retrained \texttt{r=16} model fixes the
  directional errors. Pre-augmentation: \texttt{slight\_pos} for all minority
  blocs on Newsom 2028 (wrong). After retraining on the 1,136-record augmented
  dataset, confirmed on production Modal A10 with v2 adapter + seeded constrained
  decoding: white=\texttt{slight\_neg}, latino=\texttt{slight\_neg},
  evangelical=\texttt{mod\_neg}, secular=\texttt{slight\_pos}. Direction
  differentiates correctly by bloc. REMAINING (paper obligation): run
  $\geq$3 seeded passes under the same adapter/decoding path and produce a
  headline-event results table. Latino is boundary-unstable (note this in
  the paper). If the model is wrong on any of the 3 runs, escalate to
  Espinosa before writing the results section.}{3}
...
\task{DONE: Implement the \texttt{/estimate/stream} SSE endpoint in
  \texttt{shock\_endpoint.py}. FastAPI \texttt{StreamingResponse} with
  \texttt{media\_type="text/event-stream"} is deployed and working on the
  live Modal A10. Named events \texttt{deltas}, \texttt{equilibrium},
  \texttt{simulation}, and \texttt{done} are emitted in order as each
  pipeline stage completes. \texttt{page.tsx} opens an \texttt{EventSource}
  and populates state via \texttt{onEquilibrium}/\texttt{onSimulation}
  callbacks with Zod~v4 schema validation at the SSE boundary.
  Progressive-render UX confirmed working end-to-end (charts populate
  incrementally as each stage returns). OPEN: SSE ``30\% latency
  improvement'' claim is incorrect (see latency flag in the log above ---
  streaming improves time-to-first-content, not total round-trip); update
  devplan copy accordingly.}{3}
...
\task{Resolve the meaning of \texttt{target}. PARTLY DONE. (1) The VALUE is
  principled: \texttt{configs/party\_config.json} defines V\_eq = 0.5066 (Dem) /
  0.4934 (Rep) as the EC-adjusted popular-vote share where P(EC win)=0.5, derived
  by \texttt{derive\_ec\_veq()} (logistic fit over 20 cycles). (2) RECONCILED: the
  hardcoded \texttt{\_V\_EQ\_DEFAULT} fallback now matches (0.5066/0.4934) instead
  of the stale 0.521/0.495. \texttt{base.json} already agreed at 0.5066;
  \texttt{smoke.json} deliberately keeps 0.535 as a fast-test value. (3) FIXED a
  real bug: the validators required \texttt{0.5 < target < 0.7}, which rejected the
  Republican EC V\_eq (0.4934 < 0.5) and broke every Republican run --- widened to
  \texttt{(0.40, 0.70)} in PipelineConfig / BaselinePortfolioData / EquilibriumData,
  with regression tests. NOTE: the earlier hypothesis that 0.5066 is ``too low''
  and causes \texttt{win\_probability=1.0} saturation was WRONG --- the saturation
  comes from the near-zero DIAGONAL $\Sigma_\Delta$ (smoke has no real panel
  covariance; $\mu_{eff}\approx0.59 \gg 0.5066$ with tiny variance $\Rightarrow$
  every draw wins), tracked by the ``production covariance path'' task.
  REMAINING (needs Espinosa, a modeling call not a code fix): is $\mu_{eff}$ --- a
  $\lambda$-weighted blend over three overlapping strata --- a fair proxy for the
  national two-party popular-vote share that V\_eq thresholds? Document the
  approximation in DECISIONS.md.}{2}
...
\task{Confirm the production covariance path. DONE: \texttt{run\_ilr\_montecarlo}
  now accepts a \texttt{cov\_delta} argument and uses it as $\Sigma_\Delta$ (with
  PSD repair); the real shock covariance is threaded through every caller --- the
  SSE API (\texttt{\_run\_mc\_thread}), the Monte Carlo CLI, and the pipeline
  stage (\texttt{stages.run\_simulations} $\leftarrow$ \texttt{pipeline.py}). When
  \texttt{cov\_delta} is None it falls back to the isotropic diagonal, so existing
  behavior is preserved (tests in \texttt{test\_montecarlo.py}). STILL TO DO:
  ensure the $\Sigma_\Delta$ actually SUPPLIED is the genuine Ledoit-Wolf estimate
  from 14 real cycle-deltas, not itself the diagonal fallback (the smoke artifact's
  covariance is currently $0.02^2 I$ because no panel parquet was present). Add a
  variance FLOOR so a near-zero empirical $\Sigma_\Delta$ can't collapse the CI to
  $[1.0,1.0]/[0.0,0.0]$, log which covariance source was used, and add a test
  asserting the fallback is NOT silently used when real panel data is present.}{2}
...
\task{Verify no JSON artifact can contain \texttt{Infinity}/\texttt{NaN}.
  The \texttt{eval\_mae=inf} bug already produced one invalid-JSON parse failure.
  DONE: a shared \texttt{sanitize\_floats()} sanitizer in
  \texttt{electoral/core/io.py} (recursively maps inf/-inf/nan $\to$ null), tested
  in \texttt{tests/test\_io.py::TestSanitizeFloats}, is now applied at EVERY JSON
  emission point we control --- \texttt{write\_json}/\texttt{write\_artifact}, the
  Monte Carlo CLI (\texttt{simulation/montecarlo.py}), the SSE frames
  (\texttt{api/shock\_endpoint.py}), and the fine-tune record serializer
  (\texttt{llm/trainer.py}). REMAINING: \texttt{trainer\_state.json} is written by
  the HuggingFace \texttt{Trainer} itself (not our code), so a non-finite
  \texttt{best\_metric} there isn't caught by our sanitizer. Close it with a
  \texttt{TrainerCallback} that sanitizes the metrics dict (or post-process
  \texttt{trainer\_state.json} after training), and stop \texttt{\_eval\_mae} from
  propagating a raw \texttt{inf} into the saved state.}{2}

\task{DONE: Enforce per-bloc weight bounds inside the optimizer instead of after
  it. \texttt{solve\_dqcp} (electoral/optimization/dqcp.py) now enforces the
  \texttt{ConstraintSpec} bounds EXACTLY inside the SOCP via the linear pair
  \texttt{z\_i >= lo\_i*s} / \texttt{z\_i <= hi\_i*s} (added as
  \texttt{cp.multiply(lo, s)} constraints), replacing the old post-solve
  clip+renorm that could push a weight back outside its bound. Also fixed a latent
  attribute bug (the old code read \texttt{spec.lower\_bounds}/\texttt{upper\_bounds};
  the real fields are \texttt{spec.lower}/\texttt{spec.upper}). Tests added in
  \texttt{tests/test\_optimization.py} assert the upper cap and lower floor hold
  for specs the unconstrained optimum violates. Behavior-preserving for current
  runs (no caller passes a \texttt{spec} yet, so the spec branch was dead code).}{3}

\task{DONE: Reconcile the religion/gender basis in $\mu_{\text{eff}}$. The
  post-shock optimizer and Monte Carlo no longer use a flat neutral 0.5 for the
  fixed strata. A shared \texttt{dqcp.compute\_fixed\_loyalty()} computes the REAL
  religion+gender contribution
  $\lambda_2\!\cdot\!\overline{\mu_{rel}} + \lambda_3\!\cdot\!\overline{\mu_{gen}}$
  from the baseline loyalties shifted by the shock's religion/gender deltas
  (post-shock-shifted, consistent with how race uses $\tilde\mu$). The kernel
  passes it (from the baseline artifact); the API passes it (from nominal priors
  \texttt{\_NOMINAL\_MU\_RELIGION/GENDER}); \texttt{run\_ilr\_montecarlo} DERIVES
  it from \texttt{mu\_eff\_shifted} so the MC's win check always matches the
  optimizer. \texttt{solve\_rebalanced} / \texttt{run\_ilr\_montecarlo} take an
  opt-in \texttt{fixed\_loyalty} (None $\to$ old neutral, behavior-preserving).
  Smoke impact: $\mu_{\text{eff}}$ 0.5959$\to$0.5900, weights shift $\le 0.008$.
  Tests in \texttt{test\_optimization.py} / \texttt{test\_montecarlo.py}.
  STILL SEPARATE (do not conflate): (a) the \texttt{v\_R}/\texttt{g\_G} within-
  stratum weights are still equal (1/N) until \texttt{raking.py} supplies real
  panel marginals; (b) the \emph{value} of \texttt{V\_eq}/\texttt{target} (EC vs
  popular-vote definition) is the open ``resolve target'' task --- the basis fix
  here does NOT change it. Re-freeze the baseline once both are settled, and
  document in DECISIONS.md.}{3}

\task{DONE: Consolidate the two duplicate optimizer code paths. There used to be
  two SOCP formulations that solved DIFFERENT problems: the kernel's
  \texttt{cvx.solve\_rebalanced} compared race-only $\mu\cdot w$ to $V_{eq}$ (no
  $\lambda$ weighting, but it DID enforce the [0.05, 0.60] bounds), while the
  API's wrapper used \texttt{dqcp.solve\_dqcp} with the full $\lambda$-weighted
  $\mu_{\text{eff}}$ but NO bounds. Now: \texttt{dqcp.solve\_dqcp} is the single
  canonical solver; \texttt{cvx.solve\_rebalanced} is a thin wrapper that calls it
  (with a [floor, ceiling] \texttt{ConstraintSpec}) and returns an
  \texttt{EquilibriumData}; the FastAPI wrapper delegates to
  \texttt{cvx.solve\_rebalanced}. Both paths now use the SAME $\lambda$-weighted
  objective AND the same per-bloc bounds, and \texttt{mu\_eff\_shifted} is
  populated (was unset on the kernel path). Smoke-input impact: coalition weights
  move $\le 0.016$; \texttt{mu\_eff\_shifted} 0.0$\to$0.5959. Committed
  \texttt{artifacts/smoke/*} still reflect the OLD kernel optimizer and will
  update on the next real pipeline run (regen needs the LLM; not hand-edited).
  Religion/gender are still held at neutral 0.5 --- switching them to the real
  loyalties is the remaining ``$\mu_{\text{eff}}$ basis'' task below.}{3}
