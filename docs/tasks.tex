...
\begin{taskbox}
\done \task{Resolve substantive open questions at the Week~7 supervision
  check-in with Prof.\ Espinosa. Three flag-log items cannot close without
  his answers; get them on record before touching the code:
  \begin{itemize}[leftmargin=1.6em, itemsep=1pt]
    \item \textbf{What does \texttt{target=0.5066} represent?} Popular-vote
      share? EC threshold? Effective-loyalty cutoff? The answer determines
      whether win\_probability=1.0 saturation is a bug or a modeling choice,
      and whether the current value is defensible.
    \item \textbf{What should win\_probability\_low/high measure?} Monte
      Carlo sampling error (the current code) or $\Sigma_\Delta$
      covariance-propagated uncertainty (what the methodology section
      implies)? These are different intervals with different scientific
      meanings; the choice must be explicit and documented.
    \item \textbf{Is the model-vs-market juxtaposition framed honestly?}
      The model answers $P(\text{win} \mid \text{hypothetical shock})$; the
      market answers $P(\text{win} \mid \text{current reality})$. Confirm
      the UI copy and methodology text distinguish these, and that no
      generic election contract is presented as event-conditioned.
  \end{itemize}
  Capture his answers in \texttt{DECISIONS.md} and mark the three
  corresponding flag-log checkboxes ``resolved per Espinosa \textit{<date>}''
  or ``deferred per Espinosa \textit{<date>}''. Cosmetic feedback (bloc
  display names, axis labels, disclaimer wording) is the easy part of
  this task; the three above are the substantive gate. Re-run the manual
  end-to-end test after each cosmetic change. Commit as
  \texttt{fix: incorporate-supervision-feedback}.}{3}
