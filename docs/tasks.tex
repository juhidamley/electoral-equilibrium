...
\task{Flag-log entries captured so far (append below as new ones arise):
  \begin{itemize}[leftmargin=1.6em, itemsep=1pt]
    ...
    \item[$\square$] \textbf{[blocking --- deploy]} Constrained decoding is
      silently broken: \texttt{\_BinToken = Literal[tuple(DELTA\_BINS)]}
      fails under Pydantic 2.13 (``Unable to generate pydantic-core schema
      for Literal[\ldots]''), so \texttt{outlines} never engages and the
      schema-validity guarantee is OFF in production. The greedy fallback
      also crashes (\texttt{format\_prompt} arity mismatch). Fix: (a)
      Literal schema construction for current Pydantic; (b)
      \texttt{format\_prompt()} call signature in \texttt{\_predict\_greedy}.
      The deployed endpoint cannot promise valid structured output until
      constrained decoding actually runs.
