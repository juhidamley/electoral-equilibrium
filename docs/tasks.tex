...
\task{Flag-log entries captured so far (append below as new ones arise):
  \begin{itemize}[leftmargin=1.6em, itemsep=1pt]
    ...
    \item[$\boxtimes$] \textbf{[blocking --- RESOLVED for MVP 2026-06-23]}
      Newsom directional error fixed for white (stable slight\_neg across
      runs). Latino direction is \emph{model-boundary unstable}: varies
      across runs (slight\_neg / slight\_pos / neutral depending on decode
      path and seed). This is not a residual bug but a genuine model
      uncertainty at the decision boundary for this bloc/event combination.
      No paper-grade directional claim about latino on Newsom may be made
      until generation is seeded and the seeded constrained result is stable.
      See decode-sensitivity flag.
    ...
    \item[$\square$] \textbf{[correctness]} win\_probability CI measures
      the wrong uncertainty --- both bootstrap variants compute Monte Carlo
      sampling error (shrinks with $N$, not scientifically meaningful),
      not covariance-propagated uncertainty ($\Sigma_\Delta$ sampling
      distribution through ILR). Decide with Espinosa: (a) sample
      $\Sigma_\Delta$ from a Wishart and propagate through full ILR MC,
      or (b) explicitly redefine the fields as sampling-error bounds and
      document that covariance uncertainty is out of scope. Do not present
      a Bernoulli CI as the covariance-propagated interval the methodology
      section claims.
