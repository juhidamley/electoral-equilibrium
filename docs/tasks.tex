...
\task{Enforce per-bloc weight bounds inside the optimizer instead of after it.
  \texttt{solve\_dqcp} (electoral/optimization/dqcp.py) currently applies the
  \texttt{ConstraintSpec} lower/upper bounds by clipping the solved weights and
  then renormalising so they sum to 1. The renormalisation step can push a
  clipped weight back outside its bound (e.g.\ clip to the ceiling, then divide
  by a sum $<1$, which scales it above the ceiling), so the returned weights are
  not guaranteed to respect \texttt{spec}. This is flagged in-code as a known
  first-order approximation. Replace it with a formulation that enforces the
  bounds exactly: in the Charnes--Cooper SOCP variables this is the linear pair
  \texttt{z\_i >= lo\_i * s} and \texttt{z\_i <= hi\_i * s} (bounds become linear
  in $(z,s)$, no clipping needed). Add a test asserting every returned weight is
  within \texttt{[lo, hi]} for a spec that the unconstrained optimum violates.}{3}

\task{Reconcile the religion/gender basis in $\mu_{\text{eff}}$ between the
  baseline and the post-shock stages. \texttt{kernels/baseline.py} computes the
  baseline $\mu_{\text{eff}}$ using the REAL religion and gender vote shares
  (\texttt{mu\_religion}, \texttt{mu\_gender}, equal-weighted per bloc), but the
  post-shock optimizer (\texttt{optimization/dqcp.py}, \texttt{cvx.py}) and the
  Monte Carlo (\texttt{simulation/montecarlo.py}) both treat those two strata as
  a flat neutral 0.50 (\texttt{fixed\_loyalty} / \texttt{neutral\_fixed}). The two
  $\mu_{\text{eff}}$ numbers are therefore on different bases, so any
  ``baseline $\to$ post-shock'' shift the app or paper reports conflates the real
  shock effect with this 0.5-vs-real discrepancy. Decide which convention is
  canonical (likely: use the real fixed-strata loyalties everywhere, or hold them
  fixed but identical in both places), apply it consistently, document it in
  DECISIONS.md, and re-freeze the baseline.}{3}

\task{Consolidate the two duplicate optimizer code paths. There are now two
  separate SOCP formulations of the same max-P(win) problem:
  \texttt{electoral/optimization/cvx.py:solve\_rebalanced} (used by the
  \texttt{build\_shock\_response} kernel) and
  \texttt{electoral/optimization/dqcp.py:solve\_dqcp} (used by the FastAPI
  \texttt{/estimate/stream} route via a wrapper). They can diverge silently and
  double the maintenance/test burden. Pick one as canonical (dqcp.py is the more
  general and better-documented), have the other delegate to it or be removed,
  and point every caller (kernel, API, sensitivity\_analysis) at the single
  implementation. Confirm the smoke artifacts are unchanged afterward.}{3}
