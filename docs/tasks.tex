...
\task{Flag-log entries captured so far (append below as new ones arise):
  \begin{itemize}[leftmargin=1.6em, itemsep=1pt]
    ...
    \item[$\square$] \textbf{[blocking --- public deploy]} No rate
      limiting or spend cap on the unauthenticated GPU endpoint; the
      share-link feature lets strangers trigger \textasciitilde12\,s GPU
      calls. Add per-IP rate limit, Modal concurrency cap, and billing
      alert before going public.
    \item[$\square$] \textbf{[correctness --- CONFIRMED OPEN 2026-06-24]}
